  ...
  const initThreeJS = () => {
    ...
    const fragmentShader = `
      ...
      void main(void) {
        ...
        // Circular fade — sphere mask (scales dramatically with intensity for audio reactivity)
        float sphereScale = 1.0 + (intensity - 1.0) * 0.25;
        float dist = length(uv) / sphereScale;
        float sphere = smoothstep(1.0, 0.45, dist);
        if (sphere < 0.001) {
        ...
        float lineWidth = 0.0008 * intensity;
        ...
        // Core glow — stronger with intensity
        float coreGlow = exp(-dist * 2.5) * 0.15 * intensity;
